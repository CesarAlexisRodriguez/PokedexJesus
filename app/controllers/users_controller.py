...
def login():
    ...
    if not email or not password:
        return jsonify("Es necesario enviar todas las crendenciales", 400)
    user = user_model.get_by_email_password(email, password)
    if not user:
        return jsonify("no se encontro ningun usuario", 400)
...
def register():
    try:
        ...
        return jsonify("Los parametros son incorrectos", 400)
